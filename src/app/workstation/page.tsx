...
// 导入 workstation 专用色板数据工具
import {
  getWorkstationBrands,
  getBrandColors,
  getBrandCodeToIdMapping,
  getBrandIdToCodeMapping,
  getMasterBrands
} from '../../utils/workstationColorUtils';

// 导入 newColorData 用于预设颜色匹配
import newColorData from '../api/palettes/newColorData.json';

// 导入组件
import PixelatedPreviewCanvas from '../../components/PixelatedPreviewCanvas';
import GridTooltip from '../../components/GridTooltip';
import PaletteCenter from '../../components/PaletteCenter';
...
// 颜色块组件 - 使用 React.memo 优化渲染性能
const ColorBlock = React.memo(function ColorBlock({
  code,
  color,
  dark = false,
  active = false,
...
// 获取 Mard 291色的色号到 hex 的映射（使用 colorSystemMapping.json，和 caidan1 仓库一致）
const mardToHexMapping = getMardToHexMapping();

// 🎯 修正：将 key 设为 mardKey (色号)，而不是 hex (颜色值)
const fullBeadPalette: PaletteColor[] = Object.entries(mardToHexMapping)
  .map(([mardKey, hex]) => {
    const rgb = hexToRgb(hex);
    if (!rgb) return null;
    return { 
      key: mardKey, // 🚨 关键：使用色号作为身份证
      hex: hex.toUpperCase(), 
      rgb, 
      mardKey 
    } as PaletteColor;
...
export default function Workstation() {
  ...
  // 去除杂色时阻止重新像素化
  const isExcludingColorRef = useRef<boolean>(false);
  ...
  const [isCanvasLocked, setIsCanvasLocked] = useState<boolean>(false); // 固定画布状态
  ...
  const [localHistoryList, setLocalHistoryList] = useState<Array<{
    id: string;
    name: string;
    timestamp: number;
    thumbnail?: string;
  ...
  const currentGridColors = useMemo(() => {
    ...
    const uniqueColorsMap = new Map<string, { key: string; color: string }>();

    mappedPixelData.flat().forEach(cell => {
      if (cell && cell.color && !cell.isExternal) {
        // 1. 🚨 强制标准化 HEX：去掉空格，转大写，如果缺 '#' 强制补齐！
        const rawColor = cell.color.trim().toUpperCase();
        const normalizedHex = rawColor.startsWith('#') ? rawColor : `#${rawColor}`;
        
        if (!uniqueColorsMap.has(normalizedHex)) {
          // 2. 第一层查表：去当前干净的色板里找
          let realColorObj = activeBeadPalette.find(c => c.hex.toUpperCase() === normalizedHex);
          
          // 3. 第二层查表：如果图片引擎瞎跑，匹配到了外面的颜色，去总表里捞！
          if (!realColorObj) {
            realColorObj = fullBeadPalette.find(c => c.hex.toUpperCase() === normalizedHex);
          }
          
          // 4. 提取真实色号：如果真查不到，把乱码前的 '#' 删掉作为兜底展示
          const realKey = realColorObj 
            ? (realColorObj.code || realColorObj.mardKey || realColorObj.key) 
            : (cell.key || rawColor.replace('#', ''));

          uniqueColorsMap.set(normalizedHex, { key: realKey, color: normalizedHex });
        }
      }
    });

    const originalColors = Array.from(uniqueColorsMap.values());
    return sortColorsByHue(originalColors);
    
  }, [mappedPixelData, activeBeadPalette, fullBeadPalette, selectedColorSystem]); // 🚨 依赖项必须包含 fullBeadPalette

  // 优化：缓存完整色板的计算结果，根据选中品牌返回对应色号
  const optimizedFullBeadPalette = useMemo(() => {
    return fullBeadPalette.map(colorItem => {
      // 根据当前选中品牌获取对应色号
      const displayKey = getDisplayColorKey(colorItem.hex, selectedColorSystem);
  ...
  }, [fullBeadPalette, selectedColorSystem]);

  // 初始化时：强制清空脏缓存，锁定 MARD 默认全量色板
  useEffect(() => {
    // 1. 🚨 强制核弹级清洗：不管以前存了什么乱七八糟的，只要刷新页面，全部清空！
    localStorage.removeItem('paletteSelections');
    localStorage.removeItem('currentPalettePreset');
    
    // 2. 重新构建最纯净的 Mard 全量名单
    const initialSelections: PaletteSelections = {};
    fullBeadPalette.forEach(c => {
      initialSelections[c.mardKey || c.key] = true;
    });
    
    // 3. 强制重置状态
    setCustomPaletteSelections(initialSelections);
    setIsCustomPalette(false);
    setSelectedColorSystem('Mard' as ColorSystem); // 强制锁定标题为 Mard
  }, []);

  // 🚨 终极修复：解决 418/291 数量偏差与连环碰撞问题
  // 🚨 关键：去除杂色时跳过，避免触发重新像素化
  useEffect(() => {
    // 🚨 去除杂色时跳过（关键检查）
    if (isExcludingColorRef.current) {
      return;
    }
    
    const dataSrc = (typeof paletteData !== 'undefined') ? paletteData : ((typeof newColorData !== 'undefined') ? newColorData : null);
    const rawBrands = (dataSrc as any)?.brands || [];
    
    const matchingBrands = rawBrands.filter((b: any) =>
      b.brandCode === selectedColorSystem ||
      b.groupName === selectedColorSystem ||
      b.brandName === selectedColorSystem
    );

    let currentBrandPalette: any[] = [];

    if (matchingBrands.length > 0) {
      // 第一阶段：在每个预设内部进行【局部清洗】（与 PaletteCenter 100% 同频）
      const presetCleanedResults = new Map<number, any[]>();
      
      matchingBrands.forEach((brandInfo: any) => {
        const rawColors = ((dataSrc as any)?.colorCards as any)[brandInfo.id] || [];
        const localSeenCodes = new Set<string>();
        const localCleaned: any[] = [];
        ...
        rawColors.forEach((c: any) => {
          ...
          let suffix = 1;
          // 🚨 核心：废除 isYuken 限制，所有品牌全部执行无差别 while 防撞名！
          ...
          // 计算 RGB 值
          const hexColor = c.color.toUpperCase();
          const rgbMatch = hexColor.match(/^#?([A-Fa-f0-9]{6})$/);
        ...
        presetCleanedResults.set(brandInfo.id, localCleaned);
      });

      // 第二阶段：跨预设【全局合并】（使用 Map 锁定 Code 唯一性）
      const finalGlobalMap = new Map<string, any>();
      
      // 按照预设大小倒序排列，确保基础数据最全
      const sortedByLength = [...matchingBrands].sort((a, b) => {
        const lenA = presetCleanedResults.get(a.id)?.length || 0;
        const lenB = presetCleanedResults.get(b.id)?.length || 0;
        return lenB - lenA;
      });

      sortedByLength.forEach(brand => {
        const list = presetCleanedResults.get(brand.id) || [];
        list.forEach(colorItem => {
          if (!finalGlobalMap.has(colorItem.code)) {
            finalGlobalMap.set(colorItem.code, colorItem);
          }
        });
      });

      currentBrandPalette = Array.from(finalGlobalMap.values());
    } else {
      currentBrandPalette = fullBeadPalette || [];
    }

    let result = [...currentBrandPalette];

    // 严格过滤逻辑：确保与 PaletteCenter 保存的名单完美匹配
    if (customPaletteSelections && Object.keys(customPaletteSelections).length > 0) {
      const selectionKeys = Object.keys(customPaletteSelections).filter(k => !k.startsWith('#'));
      
      if (selectionKeys.length > 0) {
        const matchedColors = result.filter(c => 
          customPaletteSelections[c.key] || customPaletteSelections[c.hex]
        );

        // 重合度鉴定：防止木马残留导致清空
        if (matchedColors.length >= selectionKeys.length * 0.5) {
          result = matchedColors;
        }
      }

      // 叠加自定义 HEX 颜色
      const customHexes = Object.keys(customPaletteSelections).filter(
        (k) => k.startsWith('#') && customPaletteSelections[k]
      );
      customHexes.forEach((hex) => {
        if (!result.some((c) => c.hex === hex)) {
          result.push({ key: hex, code: hex, hex: hex, mardKey: hex });
        }
      });
    }

    if (excludedColorKeys && excludedColorKeys.size > 0) {
      result = result.filter(
        (c) => !excludedColorKeys.has(c.key) && !excludedColorKeys.has(c.hex)
      );
    }
    
    // 写入状态
    setActiveBeadPalette(result);
  }, [selectedColorSystem, customPaletteSelections, excludedColorKeys, fullBeadPalette]);

  // ========== 🚨 跨品牌画布洗牌引擎：只要新色板生效，自动清洗画布上的前朝色号！ ==========
  // 🚨 关键：去除杂色时跳过，避免破坏已有的格子状态
  useEffect(() => {
    // 🚨 去除杂色时跳过
    if (isExcludingColorRef.current) {
      return;
    }
    if (!activeBeadPalette || activeBeadPalette.length === 0) return;

    setMappedPixelData((prevGrid: any[]) => {
      if (!prevGrid || prevGrid.length === 0) return prevGrid;

      let hasChanges = false;
      const remappedGrid = prevGrid.map((row: any[]) => row.map((cell: any) => {
        // 🚨 跳过透明格子
        if (!cell || cell.isExternal || !cell.color || cell.key === TRANSPARENT_KEY) return cell;

        // 1. 检查画布上的这个像素色号，是否属于当前最新切换的色板
        const isStillValid = activeBeadPalette.some(c => 
          c.key === cell.key || c.code === cell.key || c.mardKey === cell.key
        );

        // 2. 如果属于新色板，安全放行
        if (isStillValid) return cell;

        // 3. 🚨 发现前朝余孽（旧品牌的色号）！强行算色差，吸附到新品牌的珠子上！
        hasChanges = true;
        const rgbArr = hexToRgb(cell.color);
        if (!rgbArr) return cell;

        const closest = findClosestPaletteColor(rgbArr, activeBeadPalette);
        return {
          ...cell,
          color: closest.hex,
          key: closest.code || closest.mardKey || closest.key,
          mardKey: closest.mardKey || closest.code || closest.key
        };
      }));

      if (hasChanges) {
        // 同步底稿，彻底切断旧色号复活的可能
        setTimeout(() => setOriginalPixelData(remappedGrid), 0);
        return remappedGrid;
      }
      
      return prevGrid;
    });
  }, [activeBeadPalette, findClosestPaletteColor, TRANSPARENT_KEY]);
  // =================================================
  ...
  const pixelateImage = useCallback((
    ...
    img.onload = () => {
      ...
      // 🚨 终极拦截净化：遍历成千上万个格子，将野生颜色强行吸附到当前色板！
      const mappedAndCleanedGrid = mergedData.map((row: any[]) => 
        row.map((cell: any) => {
          ...
          // 2. 利用色差算法，在当前干净的色板中找一颗最接近的珠子
          const rgbArr = hexToRgb(normalizedHex);
          let closestBead = null;
          
          if (rgbArr && currentPalette && currentPalette.length > 0) {
            closestBead = findClosestPaletteColor(rgbArr, currentPalette);
          }
          
          if (closestBead) {
            // 3. 强行将这颗珠子的纯正血统（Code/HEX）注入格子！
            const finalKey = closestBead.code || closestBead.mardKey || closestBead.key;
            // 调试日志：追踪色号设置
            if (!finalKey || finalKey.startsWith('#')) {
              console.warn('[色号问题] closestBead.key 可能是 HEX:', {
                code: closestBead.code,
                mardKey: closestBead.mardKey,
                key: closestBead.key,
                finalKey,
                hex: closestBead.hex
              });
      ...
      // 更新状态 - 使用洗干净的网格
      setMappedPixelData(mappedAndCleanedGrid);
      setGridDimensions({ N, M });

      // 计算颜色统计 - 使用色号作为 key！
      const counts: { [key: string]: { count: number; color: string; realKey: string } } = {};
      let totalCount = 0;
      mappedAndCleanedGrid.flat().forEach(cell => {
        if (cell && cell.key && !cell.isExternal) {
          const colorKey = cell.key.toUpperCase();  // 🚨 使用色号作为 key！
          if (!counts[colorKey]) {
            counts[colorKey] = { count: 0, color: cell.color, realKey: cell.key };
          }
          counts[colorKey].count++;
          totalCount++;
        }
      });
  ...
  // 图片变化时触发像素化
  // 注意：依赖 activeBeadPalette 以便品牌切换时能重新像素化
  // 去除杂色时跳过重新像素化（isExcludingColorRef）
  useEffect(() => {
    // 跳过空白画布模式
    if (originalImageSrc === 'blank-canvas') {
      return;
    }
    // 去除杂色时跳过重新像素化（关键：检查 ref 而非 state）
    if (isExcludingColorRef.current) {
      console.log('[像素化] 跳过：正在去除杂色');
      return;
    }
    if (originalImageSrc && activeBeadPalette.length > 0) {
      const timeoutId = setTimeout(() => {
        // 再次检查（防止 100ms 后 ref 变回 false 时刚好触发）
        if (isExcludingColorRef.current) {
          console.log('[像素化] 跳过（延迟检查）：正在去除杂色');
          return;
        }
  ...
  }, [originalImageSrc, gridWidth, gridHeight, similarityThreshold, activeBeadPalette, pixelationMode, remapTrigger]);
  ...
  // 保存当前图纸到本地历史记录
  const handleSaveToLocalHistory = useCallback((customName?: string) => {
    ...
    const id = `history_${timestamp}`;
    const name = customName || `图纸_${new Date(timestamp).toLocaleString('zh-CN', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' })}`;
    ...
    try {
      ...
      const maxSize = 100;
      const scale = Math.min(maxSize / gridDimensions.N, maxSize / gridDimensions.M);
      canvas.width = gridDimensions.N * scale;
      canvas.height = gridDimensions.M * scale;
    ...
    const updatedList = [newHistoryItem, ...localHistoryList].slice(0, 20); // 最多保存20条
    setLocalHistoryList(updatedList);
    ...
    try {
      const serialized = updatedList.map(item => ({
        ...
        data: {
          ...item.data,
          pixelData: item.data.pixelData
        }
      }));
      localStorage.setItem('perlerHistory', JSON.stringify(serialized));
    } catch (err) {
      console.error('保存历史记录失败:', err);
    }
  }, [mappedPixelData, gridDimensions, gridWidth, gridHeight, originalImageSrc, localHistoryList]);

  // 从本地历史记录加载
  const handleLoadFromLocalHistory = useCallback((id: string) => {
    const item = localHistoryList.find(h => h.id === id);
    if (!item) {
      alert('未找到该历史记录');
      return;
    }
    ...
    const { data } = item;
    setMappedPixelData(data.pixelData);
    setGridDimensions({ N: data.gridWidth, M: data.gridHeight });
    setGridWidth(data.gridWidth);
    setGridHeight(data.gridHeight);
    setGridWidthInput(String(data.gridWidth));
    setGridHeightInput(String(data.gridHeight));
    setOriginalImageSrc(data.originalImageSrc);
    setWorkstationMode('manual');
    setIsManualColoringMode(true);
    // 初始化历史记录
    setHistory([data.pixelData]);
    setHistoryIndex(0);
  ...
  // 删除历史记录
  const handleDeleteFromLocalHistory = useCallback((id: string) => {
    const updatedList = localHistoryList.filter(h => h.id !== id);
    setLocalHistoryList(updatedList);

    // 更新localStorage
    try {
      localStorage.setItem('perlerHistory', JSON.stringify(updatedList));
    } catch (err) {
  ...
  const handleExportAsJson = useCallback(() => {
    ...
    link.href = url;
    link.download = `perler_${Date.now()}.json`;
    link.click();
    URL.revokeObjectURL(url);
  }, [mappedPixelData, gridDimensions, originalImageSrc]);

  // 从JSON文件导入图纸到历史记录
  const handleImportFromJson = useCallback((e: ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    ...
    reader.onload = (event) => {
      try {
        ...
        if (data.pixelData && data.gridDimensions) {
          // 保存到历史列表
          const timestamp = Date.now();
          const id = `history_${timestamp}`;
          const fileName = file.name.replace(/\.json$/i, '');
          const name = fileName || `图纸_${new Date(timestamp).toLocaleString('zh-CN', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' })}`;
          ...
          try {
            const canvas = document.createElement('canvas');
            const maxSize = 100;
            const scale = Math.min(maxSize / data.gridDimensions.N, maxSize / data.gridDimensions.M);
            canvas.width = data.gridDimensions.N * scale;
            canvas.height = data.gridDimensions.M * scale;
            const ctx = canvas.getContext('2d');
            if (ctx) {
              for (let row = 0; row < data.gridDimensions.M; row++) {
                for (let col = 0; col < data.gridDimensions.N; col++) {
                  const pixel = data.pixelData[row]?.[col];
                  if (pixel && pixel.color) {
                    ctx.fillStyle = pixel.color;
                    ctx.fillRect(col * scale, row * scale, scale, scale);
                  }
          ...
          const newHistoryItem = {
            ...
            thumbnail,
            data: {
              gridWidth: data.gridDimensions.N,
              gridHeight: data.gridDimensions.M,
              pixelData: data.pixelData,
              originalImageSrc: data.originalImageSrc || null
            }
          };

          const updatedList = [newHistoryItem, ...localHistoryList].slice(0, 20);
          setLocalHistoryList(updatedList);
  ...
  // 像素化模式变更
  // 绘制单个像素 - 接收颜色对象或字符串
  const drawPixel = useCallback((pixelData: MappedPixel[][], row: number, col: number, colorInput: { color: string; key?: string } | string | null) => {
    ...
    if (row >= 0 && row < height && col >= 0 && col < width) {
      ...
      if (colorInput === null) {
        ...
        // 🚨 强制绑定真实色号！
        const color = typeof colorInput === 'string' ? colorInput : colorInput.color;
        const realKey = typeof colorInput === 'string' ? colorInput : (colorInput.key || colorInput.color);
    ...
    switch (currentTool) {
        ...
        for (let dr = -(brushSize - 1); dr < brushSize; dr++) {
          for (let dc = -(brushSize - 1); dc < brushSize; dc++) {
            ...
            if (nr >= 0 && nr < height && nc >= 0 && nc < width) {
              newPixelData = drawPixel(newPixelData, nr, nc, selectedColor);
            }
          }
        }
        break;
      ...
      case 'picker':
        ...
        if (row >= 0 && row < height && col >= 0 && col < width) {
          const pixel = mappedPixelData[row][col];
          if (pixel.color) {
            const displayKey = pixel.key || pixel.color;
  ...
  const handleAutoRemoveBackground = useCallback(() => {
    ...
    newPixelData.flat().forEach(cell => {
      if (cell && !cell.isExternal && cell.key !== TRANSPARENT_KEY) {
        // 🚨 使用色号作为 key，而非 HEX
        const colorKey = cell.key;
        if (!newColorCounts[colorKey]) {
          newColorCounts[colorKey] = {
            count: 0,
            color: cell.color  // 保留 HEX 用于显示
          };
        }
        newColorCounts[colorKey].count++;
        newTotalCount++;
      }
    });
      ...
      if (isClick && isManualColoringMode) {
        ...
        if (currentTool === 'picker') {
          if (cellData && !cellData.isExternal && cellData.color) {
            const displayKey = cellData.key || cellData.color;
        ...
        // 区域擦除模式（批量擦除相同颜色）- 优先级最高，不受其他工具影响
        if (isEraseMode) {
  ...
  const handleColorClick = useCallback((hexKey: string) => {
    ...
    if (isExcluding) {
      ...
      // 阻止重新像素化
      isExcludingColorRef.current = true;
      ...
      for (let j = 0; j < M; j++) {
        for (let i = 0; i < N; i++) {
          ...
          if (cell && !cell.isExternal && cell.key === hexKey) {
      ...
      newMappedData.flat().forEach(cell => {
        if (cell && cell.color && !cell.isExternal && cell.key !== TRANSPARENT_KEY) {
          ...
          if (!newCounts[cellKey]) {
            newCounts[cellKey] = { count: 0, color: cellKey };
          }
          newCounts[cellKey].count++;
          newTotalCount++;
        }
      });
      ...
      // 恢复重新像素化
      setTimeout(() => {
        isExcludingColorRef.current = false;
      }, 100);
    } else {
      // 恢复颜色 - 触发完全重映射
      const nextExcludedKeys = new Set(currentExcluded);
    ...
    setIsManualColoringMode(false);
    setSelectedColor(null);
  }, [excludedColorKeys, mappedPixelData, gridDimensions, transparentColorData]);
  ...
  const findColorData = useCallback((hexColor: string) => {
    // 在 fullBeadPalette 中查找对应的颜色
    const colorItem = fullBeadPalette.find(c => c.hex.toUpperCase() === hexColor.toUpperCase());
    // 根据当前选中品牌获取对应色号
    const key = getDisplayColorKey(hexColor, selectedColorSystem);
    return {
      key,
      color: hexColor
    };
  ...
  const sortedColorCounts = useMemo(() => {
    ...
    return Object.entries(colorCounts)
      ...
      .map(([cellKey, data]) => {
        // 🚨 抛弃旧字典 mardToHex，直接在当前色板中查找真实颜色！
        const paletteColor = activeBeadPalette.find(c => 
          c.key === cellKey || c.code === cellKey || c.mardKey === cellKey
        );
        return { 
          key: cellKey, 
          color: paletteColor ? paletteColor.hex : data.color, 
          count: data.count 
        };
      });
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(135deg,rgba(255,245,250,0.95)_0%,rgba(245,243,255,0.95)_50%,rgba(240,249,255,0.95)_100%)] p-3 text-slate-800 sm:p-4 lg:p-5" style={{ touchAction: 'pan-y' }}>
        ...
        <div className="hidden md:flex items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            <button
              ...
              className="px-3.5 py-2 text-sm font-semibold rounded-2xl bg-white/80 text-slate-600 shadow-[0_8px_18px_rgba(30,41,59,0.05)] hover:bg-white hover:shadow-md transition-all border border-white/60 flex items-center gap-1.5"
            >
        ...
        <div className="md:hidden flex flex-col gap-2 py-2">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <button
              ...
              className="h-8 px-3 text-[10px] font-medium text-white bg-gradient-to-r from-purple-500 to-pink-500 rounded-xl shadow-sm hover:shadow-md transition-all flex items-center gap-1"
          ...
          <div className="grid grid-cols-4 gap-1.5">
            ...
            <button
              onClick={() => setIsHistoryModalOpen(true)}
              className="h-8 px-1 text-[9px] font-medium text-white bg-gradient-to-r from-orange-500 to-pink-500 rounded-xl shadow-sm hover:shadow-md transition-all flex flex-col items-center justify-center gap-0.5"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
      ...
      {/* 主内容区域 - 移动端上下布局，桌面端布局 */}
      {/* 预处理和精修模式：两列布局；一体化工作台模式：三列布局 */}
      <div className={`mt-14 md:mt-16 flex flex-col md:flex-row gap-2.5 ${viewMode === 'workspace' ? 'xl:gap-4' : ''}`}>
        {/* 画布区域 - 移动端上半屏，桌面端中间区域 */}

        {/* 一体化工作台：左侧预处理工具栏 */}
        {viewMode === 'workspace' && (
          <aside className="hidden md:block h-[calc(100vh-180px)] md:h-[calc(100vh-120px)] md:w-[340px] xl:w-[340px] 2xl:w-[360px] md:flex-shrink-0 overflow-hidden">
            <div className="h-full overflow-y-auto p-4 space-y-4">
              ...
              {/* 去除杂色面板 */}
              {sortedColorCounts && sortedColorCounts.length > 0 && (
        ...
        <main ref={mainRef} className={`h-[calc(100vh-200px)] md:h-[calc(100vh-100px)] overflow-hidden bg-transparent ${viewMode === 'workspace' ? 'md:flex-1' : 'md:flex-1'}`}>
          {!originalImageSrc ? (
            <>
              {/* 桌面端：西瓜Logo预览 */}
            ...
            <div className="h-full flex flex-col p-0.5 sm:p-1">
              ...
              {/* 画布容器 - 增强玻璃态效果 */}
              <div className="flex-1 flex items-center justify-center bg-white/40 backdrop-blur-2xl rounded-[32px] border border-white/60 shadow-[0_8px_32px_rgba(31,38,135,0.07),inset_0_2px_4px_rgba(255,255,255,0.8)] overflow-hidden p-0.5 sm:p-1 relative">
                {/* 固定画布按钮 - 电脑端右上角 */}
                {isHighlightMode && (
                  <button
                    onClick={() => setIsCanvasLocked(!isCanvasLocked)}
                    className="hidden md:flex absolute top-3 right-3 z-20 items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-amber-400 to-orange-500 hover:from-amber-500 hover:to-orange-600 text-white rounded-full shadow-md transition-all text-xs font-medium"
                  >
                    {isCanvasLocked ? (
                      <>
                ...
                <PixelatedPreviewCanvas
                  ...
                  showReferenceLayer={!!originalImageSrc && originalImageSrc !== 'blank-canvas'}
                  isLocked={isHighlightMode && isCanvasLocked}
                />
              </div>
                ...
                <div className="hidden md:grid grid-cols-3 gap-3 mt-1">
                  ...
                  <div className="rounded-[22px] border border-white/75 bg-white/72 p-3 shadow-[0_8px_18px_rgba(30,41,59,0.04)]">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div className="text-sm font-semibold text-slate-800">当前工具</div>
                    </div>
                    <div className="text-[11px] text-slate-400 mb-1">正在编辑</div>
                    <div className="text-lg font-bold text-slate-700">
                      {currentTool === 'hand' ? '拖拽' : currentTool}
                    </div>
                  </div>
                  ...
                  <div className="rounded-[22px] border border-white/75 bg-white/72 p-3 shadow-[0_8px_18px_rgba(30,41,59,0.04)]">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div className="text-sm font-semibold text-slate-800">笔刷大小</div>
                    </div>
                    <div className="text-[11px] text-slate-400 mb-1">当前参数</div>
                    <div className="text-lg font-bold text-slate-700">
                      {brushSize}
                    </div>
                  </div>
                  ...
                  <div className="rounded-[22px] border border-white/75 bg-white/72 p-3 shadow-[0_8px_18px_rgba(30,41,59,0.04)]">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div className="text-sm font-semibold text-slate-800">网格线间隔</div>
                    </div>
                    <div className="text-[11px] text-slate-400 mb-1">辅助设置</div>
                    <div className="text-lg font-bold text-slate-700">
                      {gridLineInterval}
                    </div>
                  </div>
        ...
        <div className="md:hidden fixed inset-x-3 bottom-3 z-30 sm:inset-x-4">
          <div className="p-2 rounded-[28px] border border-white/70 bg-white/95 shadow-[0_18px_55px_rgba(30,41,59,0.08)]">
            {/* 高亮模式固定画布按钮 */}
            {isHighlightMode && (
              <button
                onClick={() => setIsCanvasLocked(!isCanvasLocked)}
                className="w-full mb-2 flex items-center justify-center gap-2 py-2.5 px-4 bg-gradient-to-r from-amber-400 to-orange-500 hover:from-amber-500 hover:to-orange-600 text-white rounded-full shadow-md transition-all text-sm font-medium"
              >
                {isCanvasLocked ? (
                  <>
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <AnimatePresence>
          {isMobilePanelOpen && (
            <div className="md:hidden fixed inset-0 z-40 flex items-end">
              <button className="absolute inset-0 bg-slate-900/20" onClick={() => setIsMobilePanelOpen(false)} />
              <motion.div
                initial={{ y: '100%' }}
                animate={{ y: 0 }}
                exit={{ y: '100%' }}
                ...
                layout
                className="relative z-10 max-h-[78vh] w-full overflow-hidden rounded-t-[32px] border border-white/70 bg-white/95 shadow-[0_-18px_55px_rgba(30,41,59,0.12)] will-change-transform"
              >
              <div className="sticky top-0 z-10 border-b border-slate-200/70 bg-white px-4 py-3">
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    className="flex h-10 w-10 items-center justify-center rounded-2xl border border-white/75 bg-white text-slate-500 shadow-[0_8px_18px_rgba(30,41,59,0.06)]"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <Panel
                      ...
                      right={
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-xs font-medium text-slate-600">当前: {selectedColor ? selectedColor.key : '-'}</span>
                        </div>
                      }
                    >
                    ...
                    >
                      <div className="space-y-4">
                        ...
                        <div className="grid max-h-[300px] grid-cols-5 gap-2 overflow-y-auto pr-1 sm:grid-cols-6">
                          {activeBeadPalette
                            .filter((colorItem) => {
                              const displayKey = colorItem.key || colorItem.code || colorItem.mardKey;
                      ...
                      <div className="space-y-4">
                        <div className="rounded-[20px] bg-[linear-gradient(180deg,#f7f9fd,#eef2f8)] px-4 py-3 text-center text-[15px] font-bold text-slate-700 shadow-[inset_0_1px_0_rgba(255,255,255,0.96)]">
                          {colorReplaceState.isActive && colorReplaceState.step === 'select-target'
                            ? `选择替换颜色（原色：${colorReplaceState.sourceColor ? colorReplaceState.sourceColor.key : ''}）`
                            : '点击颜色替换为想要的颜色'}
                        </div>
                        <input
                          className="h-11 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm outline-none placeholder:text-slate-400"
                        ...
                        <div className="grid max-h-[280px] grid-cols-5 gap-2 overflow-y-auto pr-1 sm:grid-cols-6">
                          {colorReplaceState.isActive && colorReplaceState.step === 'select-target'
                            ? activeBeadPalette
                                .filter((colorItem) => {
                                  const itemKey = colorItem.key || colorItem.code || colorItem.mardKey;
                                  if (!colorReplaceSearchQuery) return true;
                                  return itemKey.toUpperCase().includes(colorReplaceSearchQuery) || colorItem.hex.toUpperCase().includes(colorReplaceSearchQuery);
                                })
                                .map((colorItem) => {
                                  const itemKey = colorItem.key || colorItem.code || colorItem.mardKey;
                                  const rgb = hexToRgb(colorItem.hex);
                                  ...
                                  return (
                                    <ColorBlock
                                      ...
                                      color={colorItem.hex}
                                      dark={!isLight}
                                      onClick={() => handleColorReplace(colorReplaceState.sourceColor!, { key: itemKey, color: colorItem.hex })}
                                      title={itemKey}
                                    />
                                  );
                                })
                                  ...
                                  return (
                                    <ColorBlock
                                      ...
                                      onClick={() => {
                                        ...
                                        setColorReplaceState({ isActive: true, step: 'select-target', sourceColor: { key: displayKey, color: sourceHex } });
                                      }}
                                      title={displayKey}
                                    />
                                  );
        ...
        {/* 功能面板 - 仅桌面端显示 */}
        {(viewMode === 'preprocess' || viewMode === 'refine' || viewMode === 'workspace' || (workstationMode === 'manual' && originalImageSrc)) && (
          <aside className={`hidden md:block h-[calc(100vh-180px)] md:h-[calc(100vh-120px)] md:flex-shrink-0 overflow-hidden ${viewMode === 'workspace' ? 'md:w-[380px] xl:w-[380px] 2xl:w-[400px]' : 'md:w-[360px]'}`}>
            {/* 根据viewMode显示不同内容 */}
            {viewMode === 'preprocess' ? (
              /* 预处理视图 - 左侧控制面板 */
              <div className="h-full overflow-y-auto p-4 space-y-4">
              ...
              /* 一体化工作台视图 - 右侧栏 - 使用精修模式的工具栏 */
              <div className="h-full overflow-y-auto p-4 space-y-4">
              <Panel title="精细调整" icon={<Pencil className="h-5 w-5" />} right={
                <div className="flex items-center gap-3">
                  <span className="text-sm text-slate-400">当前: {currentTool === 'hand' ? '拖拽' : currentTool}</span>
                ...
                <div className="mb-4 flex gap-2">
                  <TinyIconButton icon={<Undo2 className="h-4 w-4" />} onClick={handleUndo} disabled={historyIndex <= 0} className={historyIndex <= 0 ? 'opacity-40 cursor-not-allowed' : ''} />
                  <TinyIconButton icon={<Redo2 className="h-4 w-4" />} onClick={handleRedo} disabled={historyIndex >= history.length - 1} className={historyIndex >= history.length - 1 ? 'opacity-40 cursor-not-allowed' : ''} />
                </div>
                <div className="grid grid-cols-3 gap-3">
                  {[
                    { label: '画笔', icon: Pencil, tool: 'brush' as ToolType },
              ...
              <Panel title="画笔颜色选择" icon={<Palette className="h-5 w-5" />} right={selectedColor && selectedColor.key !== TRANSPARENT_KEY ? (
                <div className="flex items-center gap-2">
                  <div
                    className="w-5 h-5 rounded border border-gray-300"
                    style={{ backgroundColor: selectedColor.color }}
                  ...
                  <div className="grid max-h-[300px] grid-cols-5 gap-2 overflow-y-auto pr-1">
                    {activeBeadPalette
                      .filter((colorItem) => {
                        const itemKey = colorItem.key || colorItem.code || colorItem.mardKey;
                        if (!colorSearchQuery) return true;
                        return itemKey.toUpperCase().includes(colorSearchQuery) || colorItem.hex.toUpperCase().includes(colorSearchQuery);
                      })
                      .map((colorItem) => {
                        const itemKey = colorItem.key || colorItem.code || colorItem.mardKey;
                        const rgb = hexToRgb(colorItem.hex);
                        ...
                        return (
                          <ColorBlock
                            ...
                            active={selectedColor?.color?.toUpperCase() === colorItem.hex.toUpperCase()}
                            onClick={() => {
                              setSelectedColor({ key: itemKey, color: colorItem.hex, isExternal: false });
                              setIsManualColoringMode(true);
                              setCurrentTool('brush');
                              setWorkstationMode('manual');
                            }}
                  ...
                  <div className="grid max-h-[280px] grid-cols-5 gap-2 overflow-y-auto pr-1">
                    {colorReplaceState.isActive && colorReplaceState.step === 'select-target'
                      ...
                      : sortedColorCounts
                          .filter(({ key }) => {
                            if (!colorReplaceSearchQuery) return true;
                            return key.toUpperCase().includes(colorReplaceSearchQuery);
                          })
                          .map(({ key, color }) => {
                            const rgb = hexToRgb(color);
                            const isLight = rgb ? (rgb.r * 299 + rgb.g * 587 + rgb.b * 114) / 1000 > 128 : false;
                            return (
                              <ColorBlock
                                ...
                                onClick={() => {
                                  const sourceHex = color.toUpperCase();
                                  setColorReplaceState({ isActive: true, step: 'select-target', sourceColor: { key: key, color: sourceHex } });
                                }}
                                title={key}
                              />
                            );
      ...
      <AnimatePresence>
        {showWelcomeModal && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[100] flex justify-center items-end sm:items-center p-0 sm:p-4"
          >
            <motion.div
              ...
              className="bg-white dark:bg-gray-900 rounded-t-3xl sm:rounded-2xl shadow-2xl w-full sm:max-w-md overflow-hidden border border-gray-100 dark:border-gray-700 max-h-[90vh] sm:max-h-[85vh] flex flex-col"
            >
              {/* 顶部装饰 - 简化手机版 */}
              <div className="bg-gradient-to-r from-orange-500 via-amber-500 to-orange-400 px-4 py-5 sm:px-6 sm:py-6 text-center flex-shrink-0">
                <div className="text-4xl sm:text-5xl mb-2">🍉</div>
                <h2 className="text-lg sm:text-xl font-bold text-white">
                  尊贵的小瓜拼豆超级SVIP会员
                </h2>
                <p className="text-white/90 text-xs sm:text-sm mt-0.5">欢迎使用本产品</p>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h3 className="text-sm font-semibold text-gray-800 dark:text-white">使用须知</h3>
                </div>
                ...
                <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-3 border border-blue-100 dark:border-blue-800/50">
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      <p className="font-semibold text-blue-700 dark:text-blue-300 text-xs mb-0.5">智能优化功能</p>
                      <p className="text-gray-600 dark:text-gray-400 text-xs leading-relaxed">
                        如果您不知道如何调整，请点击<span className="text-blue-600 font-medium">「智能优化」</span>按钮，系统会自动帮您优化。
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {/* 注意事项2 */}
                <div className="bg-purple-50 dark:bg-purple-900/20 rounded-xl p-3 border border-purple-100 dark:border-purple-800/50">
                  <div className="flex items-start gap-2.5">
                    <div className="w-6 h-6 rounded-lg bg-purple-500 flex items-center justify-center flex-shrink-0">
                      <span className="text-white text-xs font-bold">2</span>
                    </div>
                    <div>
                      <p className="font-semibold text-purple-700 dark:text-purple-300 text-xs mb-0.5">浏览器使用要求</p>
                      <p className="text-gray-600 dark:text-gray-400 text-xs leading-relaxed">
                        请<strong className="text-purple-600">复制到浏览器</strong>打开使用：
                      </p>
                      <div className="mt-1.5 flex flex-wrap gap-1.5">
                        <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-purple-100 dark:bg-purple-800/50 text-purple-700 dark:text-purple-300 text-xs">
                          📱 安卓：QQ浏览器
                        </span>
                        <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-pink-100 dark:bg-pink-800/50 text-pink-700 dark:text-pink-300 text-xs">
                          🍎 苹果：百度APP
                        </span>
                      </div>
                      <p className="text-amber-600 dark:text-amber-400 text-xs mt-1.5 bg-amber-50 dark:bg-amber-900/30 rounded-lg p-1.5">
                        ⚠️ 苹果100尺寸异常时，改为95/90
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-green-50 dark:bg-green-900/20 rounded-xl p-3 border border-green-100 dark:border-green-800/50">
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      <p className="font-semibold text-green-700 dark:text-green-300 text-xs mb-0.5">真人图片处理</p>
                      <p className="text-gray-600 dark:text-gray-400 text-xs leading-relaxed">
                        真人图片请先转换成<span className="text-green-600 font-medium">卡通图</span>再使用。直接转换效果不理想是正常的。
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {/* 底部按钮 - 固定在底部 */}
              <div className="px-4 pb-4 pt-2 flex-shrink-0 bg-white dark:bg-gray-900">
                <button
                  onClick={() => {
                    localStorage.setItem('xiaogua_welcome_shown', 'true');
                    setShowWelcomeModal(false);
                  }}
                  className="w-full py-3 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-orange-500/30 active:scale-[0.98]"
                >
                  我已知晓，开始使用
                </button>
              </div>
      ...
      <DownloadSettingsModal
        ...
        onSaveToHistory={handleSaveToLocalHistory}
        canSaveToHistory={!!mappedPixelData && !!gridDimensions}
      />
      ...
      {isHistoryModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-center justify-center p-2 sm:p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl sm:max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
            {/* 标题栏 */}
            <div className="flex items-center justify-between px-3 sm:px-4 py-3 border-b border-gray-100 dark:border-gray-700">
              <h3 className="text-base sm:text-lg font-bold text-gray-800 dark:text-gray-200">历史图纸</h3>
              <button
            ...
            {/* 操作按钮行 */}
            <div className="px-3 sm:px-4 py-2.5 sm:py-3 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50">
              <div className="flex flex-wrap gap-2">
                <label className="flex-1 min-w-[100px]">
                  ...
                  />
                  <div className="h-10 flex items-center justify-center gap-1.5 text-xs sm:text-sm font-semibold text-slate-700 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-all cursor-pointer shadow-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                    </svg>
                    导入图纸
                ...
                <button
                  ...
                  disabled={!mappedPixelData || !gridDimensions}
                  className="flex-1 min-w-[100px] h-10 flex items-center justify-center gap-1.5 text-xs sm:text-sm font-semibold text-slate-700 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                  </svg>
            ...
            <div className="flex-1 overflow-y-auto p-3 sm:p-4">
              {localHistoryList.length === 0 ? (
                <div className="h-full min-h-[200px] flex flex-col items-center justify-center text-center py-8">
                  <div className="text-4xl sm:text-5xl mb-3 opacity-50">📋</div>
                  <p className="text-gray-500 dark:text-gray-400 text-sm">暂无历史记录</p>
                  <p className="text-gray-400 dark:text-gray-500 text-xs mt-1.5">生成图纸后可保存到本地</p>
                </div>
              ) : (
                <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3">
                  {localHistoryList.map((item) => (
                    <div
                      key={item.id}
                      className="flex flex-col bg-white dark:bg-gray-700/50 rounded-xl border border-gray-100 dark:border-gray-600 overflow-hidden hover:shadow-lg transition-all"
                    >
                      {/* 缩略图 */}
                      ...
                      <div className="p-2 sm:p-3">
                        <p className="text-xs sm:text-sm font-medium text-gray-800 dark:text-gray-200 truncate">{item.name}</p>
                        <p className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                          {item.data.gridWidth} × {item.data.gridHeight} 格
                        </p>
                        ...
                        <div className="flex gap-1.5 mt-2">
                          <button
                            onClick={() => handleLoadFromLocalHistory(item.id)}
                            className="flex-1 py-1.5 text-[10px] sm:text-xs font-medium text-white bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 rounded-md transition-all"
                          >
                            加载
                          </button>
                          <button
                            onClick={() => {
                              if (confirm('确定要删除这条历史记录吗？')) {
                                handleDeleteFromLocalHistory(item.id);
                              }
                            }}
                            className="px-2 py-1.5 text-[10px] sm:text-xs font-medium text-red-500 bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/30 rounded-md transition-colors"
                          >
                            删除
                          </button>
                        </div>
            ...
            {/* 底部说明 */}
            <div className="px-3 sm:px-4 py-2 border-t border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50">
              <p className="text-[10px] sm:text-xs text-gray-400 dark:text-gray-500 text-center">
                历史记录保存在本地浏览器中，最多保存20条
              </p>
            </div>
          </div>
        </div>
      ...
      {/* 自定义色板编辑器弹窗 - 使用 PaletteCenter */}
      {isCustomPaletteEditorOpen && (
        <PaletteCenter
          isOpen={true}
          selectedBrand={selectedColorSystem as any}
          selectedPresetId={null}
          onClose={() => setIsCustomPaletteEditorOpen(false)}
          onSave={(colors, brand, presetId) => {
            const newSelections: PaletteSelections = {};
            
            // 🚨 直接使用 PaletteCenter 传回来的准确色号（Code），避免一切哈希冲突！
            Object.keys(colors).forEach(code => {
              newSelections[code] = true;
            });
            
            if (presetId && presetId !== 'custom' && presetId !== '全部') {
              localStorage.setItem('currentPalettePreset', presetId);
            } else {
              localStorage.removeItem('currentPalettePreset');
            }
